...
| A-7 | MED | **No flattening** — `src.City.Region.Country.DisplayOrder → CountryDisplayOrder` is not a convention. | Deliberately declined (see the Stage A scope note): flattening re-imports the invisible two-level reach this effort exists to remove, and in the view direction it triggers lazy loads. A5's synthesized `ForList(…)` line is the answer instead. **The case-sensitivity half of this gap has been split out to A-13** — it is a different problem with a different answer, and bundling them hid a cheap fix behind a deliberate decline. | A5 (message only) |
...
| A-13 | HIGH | **Member matching is case-sensitive ordinal, where AutoMapper's is not.** Every lookup is a `ToDictionary(p => p.Name, …)` with the default comparer, so `CompanyID` ↔ `CompanyId` does not match and the member is silently absent from that direction. **This is a parity regression, not a limitation:** proven by the pre-migration profile — `CompanyBranchListDTO.CompanyId` / `CityId` / `RegionId` (`string?`) mapped from entity `CompanyID` / `CityID` / `RegionID` (`long?`) **with no `ForMember` at all**, so AutoMapper both matched across case *and* converted `long?→string`. On flip, all three silently stopped projecting. | **Five** name-keyed dictionaries, not the two previously recorded: `:654`, `:803`, `:1034` (view), `:1149` (entity `dtoProps`), `:1274` (list). The FK convention is the same defect in another form — `:917` and `:1287` look up `dtoProp.Name + "ID"` and `:1117` tests `EndsWith("ID", Ordinal)`, so an entity spelling it `CompanyId` gets no FK convention either. **Live and already worked around:** `CompanyBranchRepository`'s `ForList` block carries three hand-written lines and a comment for exactly this. **The consequence escalates in the list direction** — an unprojected scalar leaves an OData `$filter` with nothing to bind to, so EF inlines the whole collection-bearing projection into the `WHERE` and **cannot translate it**: a runtime failure the first time a user filters that grid. (That comment also credits `long?→string`; the list tail *does* handle that at `:1350` — the operative cause is the case mismatch alone.) *(downstream: ~14 more members across three ADP list DTOs.)* | A4c — optioned, default insensitive; conflict → skip + `SHENGEN009` |
